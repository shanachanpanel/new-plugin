    ...
    try {
        if (m.mentionedJid.includes(this.user.jid) && m.isGroup) {
            await this.send2Button(m.chat,
                isBanned ? `${namabot} tidak aktif` : banned ? 'kamu dibanned' : `Kagumi di sini, Btw ngapain manggil kangen ya bg?`,
                footer,
...
╠═〘 ${namabot} 〙 ═`.trim(), footer, 'Dana', '#viadana', 'GoPay', '#viagopay', m)
    ...
    if (new Date() * 1 - setting.status > 1000) {
        ...
        await this.setStatus(`𝚆𝚊𝚝𝚊𝚜𝚑𝚒 𝙺𝚊𝚐𝚞𝚖𝚒 || ⏲️ 𝚁𝚄𝙽 𝚃𝙸𝙼𝙴 ${uptime} || 🛰️ 𐌼σ∂ɛ: ${global.opts['self'] ? '𝙿𝚛𝚒𝚟𝚊𝚝𝚎' : setting.groupOnly ? 'Hanya Grup' : '𝚙𝚞𝚋𝚕𝚒𝚌'}`).catch(_ => _)
